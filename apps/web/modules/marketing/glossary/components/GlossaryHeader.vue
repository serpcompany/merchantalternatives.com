...
<template>
  <div class="absolute inset-0 -z-10">
    <NuxtImg
      ...
      alt="World"
      class="bg-primary h-96 w-full object-cover"
    />
  </div>
  ...
  <div class="relative flex h-96 items-center justify-center pt-20 text-center">
    ...
    <div class="absolute bottom-0 flex gap-1 py-2">
      <NuxtLink
        v-for="letter in alphabet"
        :key="letter"
        :to="`/glossary#${letter}`"
        type="button"
        class="duration-50 flex h-10 w-10 items-center justify-center text-2xl text-white/60 backdrop-blur-sm transition-all hover:bg-white/20 hover:text-white"
      >
        {{ letter }}
      </NuxtLink>
    </div>
