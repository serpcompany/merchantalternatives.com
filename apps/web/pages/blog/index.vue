<script setup lang="ts">
  const posts = [
    ...
    {
      id: 1,
      title: "When everything is going wrong but it's all ok",
      href: "/blog/second-post",
      description:
        "Illo sint voluptas. Error voluptates culpa eligendi. Hic vel totam vitae illo. Non aliquid explicabo necessitatibus unde. Sed exercitationem placeat consectetur nulla deserunt vel. Iusto corrupti dicta.",
      imageUrl:
  ...
  <div class="bg-white">
    ...
    <div class="mx-auto flex max-w-7xl gap-10 px-6 pb-20 lg:px-8">
      ...
      <div class="flex flex-col divide-y">
        <article v-for="post in posts" :key="post.id" class="py-16">
          <div class="flex gap-2">
            <NuxtLink v-for="topic in post.topics" to="/">
              <Badge>{{ topic }}</Badge>
            </NuxtLink>
          </div>
          ...
          <p class="mt-4 text-xl text-gray-600">{{ post.description }}</p>
        </article>
      </div>
    </div>
  </div>
